...
def up_scale(scale_factor):
    ...
    for port_i in range(act_port_init, act_port_end):
        if(port_i not in active_ports):
            docker_client.containers.run("hrishikeshsuresh/acts:latest", ports = {'80' : str(port_i)}, detach = True, volumes = ['acts_storage'])
...
def auto_scaling():
    ...
    if(act_port_init not in active_ports):
        docker_client.containers.run("hrishikeshsuresh/acts:latest", ports = {'80' : str(act_port_init)}, detach = True, volumes = ['acts_storage'])
